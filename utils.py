...
def load_data(data, teachers_empty_space, groups_empty_space, subjects_order):
    """
...
def set_up(num_of_columns):
    ...
    :return: matrix, free
    """
    w, h = num_of_columns, 45  # 5 (workdays) * 9 (work hours) = 60
...
def write_solution_to_file(
    ...
    empty_teachers, max_empty_teacher, average_empty_teachers = (
        empty_space_teachers_cost(teachers_empty_space)
    )
    f.write(
    ...
    for class_index, times in filled.items():
        ...
        f.write(
            "\nClassroom: {:2s}\nTime: {}".format(
                room[: room.rfind("-")], days[times[0][0] // 9]
            )
        )
        for time in times:
